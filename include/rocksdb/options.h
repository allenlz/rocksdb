...
struct DBOptions {
  ...
  // Use to control rate of compaction. It's intuition is to control the read
  // rate of files, as reads dominate the io device sometimes.
  // This feature is under investigation, and not fully covered the whole flow
  // duiring compaction, such as compaction_filter_v2 and merge util.
  // Default: nullptr
  std::shared_ptr<RateLimiter> compaction_rate_limiter;
